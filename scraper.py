...
def validateURL(url, requestdata):
    try:
        r = requests.post(url, data = requestdata, allow_redirects=True, timeout=90)
        count = 1
...
url = 'http://www3.lancashire.gov.uk/transparency/spending/SupplierPayments.asp?sysredir=y'
requestdata = {'strAction': 'exportCSV', 'strOutputFields':'service_lable', 'strOutputFields':'service_code',
...
links = soup.find('ul', 'pal_submenu').find_all('a')
for link in links:
    url = 'http://www3.lancashire.gov.uk'+link['href']
    csvYr = link['href'].split('&')[0].split('=')[-1][-4:]
    csvMth = link['href'].split('&')[0].split('=')[-1][:2]
